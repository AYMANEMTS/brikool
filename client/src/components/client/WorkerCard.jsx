...
export default function WorkerCard({ job }) {
    ...
    return (
        <div
            className="flex flex-col max-w-lg w-full h-full mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 transition-all duration-300">
            {/* Profile Image */}
            <img
                className="w-32 h-32 rounded-full mx-auto cursor-pointer border-4 border-blue-500 hover:border-blue-700 transition-all"
            ...
            {/* Name and Category */}
            <h2
                className="text-center text-2xl font-semibold mt-3 cursor-pointer text-gray-900 dark:text-white hover:text-blue-500 transition-colors"
                onClick={() => navigate(`/worker/${job?._id}`)}
            >
                {job?.userId?.name}
            </h2>
            ...
            <div className="mt-5 flex-grow">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white">{tAnnounces('description')}</h3>
            ...
            <div className="mt-2 flex flex-wrap justify-between items-center text-sm text-gray-600 dark:text-gray-300">
              <span className="pr-2 flex items-center truncate max-w-xs sm:max-w-sm md:max-w-md">
                <MapPin fontSize="small" className="w-4 h-4 text-gray-500 dark:text-gray-400"/>
                  {job?.userId?.city?.[lng]}
                </span>
                <span className="flex items-center truncate max-w-xs sm:max-w-sm md:max-w-md">
                    <Clock8 fontSize="small" className="h-4 w-4 text-gray-500 dark:text-gray-400"/>
